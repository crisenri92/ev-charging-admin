...
export default function Dashboard() {
  ...
  const onlineChargers = chargers.filter(c => c.status?.toLowerCase() !== 'offline')
  const offlineChargers = chargers.filter(c => c.status?.toLowerCase() === 'offline')
  ...
  return (
    <div className="p-6 space-y-6">
            ...
            {onlineChargers.map((c) => (
              <div key={c.id} className="flex items-center justify-between p-3 rounded-lg bg-gray-50">
                ...
                <div className="text-right">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${c.status?.toLowerCase() === 'charging' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
                    {c.status}
                  </span>
      ...
      {!loading && offlineChargers.length > 0 && (
        <div className="bg-white rounded-xl shadow p-6">
