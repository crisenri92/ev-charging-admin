...
function VoucherModal({ v, onSave, onClose }: { v: Partial<Voucher>; onSave: (d: any) => void; onClose: () => void }) {
  ...
  const handleSave = async () => {
    if (!form.code.trim()) { toast('El código es requerido', 'error'); return }
    setSaving(true)
    try {
      await onSave({ ...form, expires_at: form.expires_at || null })
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-2xl p-6 w-full max-w-md border border-gray-700">
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} disabled={saving} className="flex-1 py-2.5 bg-gray-800 text-white rounded-xl text-sm disabled:opacity-50">Cancelar</button>
          <button onClick={handleSave} disabled={saving}
            className="flex-1 py-2.5 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white font-semibold rounded-xl text-sm transition-colors">
...
export default function VouchersPage() {
  ...
  async function fetchVouchers() {
    ...
    try {
      ...
      if (!r.ok) throw new Error('Error cargando vouchers')
  ...
  async function saveVoucher(form: any) {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        toast(d.error || 'Error al guardar el voucher', 'error')
        throw new Error(d.error)
      }
      toast(modal?.id ? 'Voucher actualizado' : 'Voucher creado')
      setModal(null)
      fetchVouchers()
    } catch {
  ...
  async function deleteVoucher(id: string) {
    setDeleting(true)
    try {
      const r = await fetch('/api/vouchers', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) })
      if (!r.ok) {
        const d = await r.json().catch(() => ({}))
        toast(d.error || 'Error al eliminar el voucher', 'error')
        return
      }
  ...
  async function toggleActive(v: Voucher) {
    try {
      const r = await fetch('/api/vouchers', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: v.id, active: !v.active }) })
      if (!r.ok) { toast('Error al cambiar el estado del voucher', 'error'); return }
      toast(v.active ? 'Voucher pausado' : 'Voucher activado')
      fetchVouchers()
    } catch {
      toast('Error de red. Intenta de nuevo.', 'error')
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => deleteVoucher(deleteTarget)} disabled={deleting} className="flex-1 px-4 py-2.5 bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white rounded-lg text-sm font-medium">
