...
function ReceiptModal({ receipt, onClose }: { receipt: Receipt; onClose: () => void }) {
  return (
    <div className="fixed inset-0 bg-black/80 flex items-end justify-center z-50 px-4 pt-4" style={{ paddingBottom: "calc(5rem + env(safe-area-inset-bottom, 0px))" }}>
      <div className="bg-gray-900 rounded-3xl p-6 w-full max-w-sm text-center border border-gray-800">
        <div className="w-16 h-16 rounded-full bg-green-900/60 border-2 border-green-500 flex items-center justify-center text-3xl mx-auto mb-4">Ã¢ÂÂ¡</div>
    ...
    <div className="min-h-screen" style={{ background: '#0f172a', paddingBottom: 'calc(6rem + env(safe-area-inset-bottom, 0px))' }}>
        ...
        <div className="px-4 mb-3">
          {myReservations.map(res => (
            <div key={res.id} className="bg-blue-900/20 border border-blue-800/40 rounded-2xl px-4 py-3 flex items-center justify-between">
              <div>
                <p className="text-blue-400 text-xs font-semibold">Ã°ÂÂÂ Reserva activa</p>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => cancelReservation(res.id)}
                  className="text-xs text-gray-500 hover:text-red-400 transition-colors">Ã¢ÂÂ</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {!mapView && (<>
        ...
        <div className="px-4 space-y-6">
          ...
          {available.length === 0 && (
            <div className="bg-gray-900 rounded-2xl p-8 text-center border border-gray-800">
              <p className="text-3xl mb-3">Ã°ÂÂÂ</p>
