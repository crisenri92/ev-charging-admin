...
export async function GET(req: NextRequest) {
  ...
  // Fallback: derive from charging_sessions
  const { data: sessions } = await supabase
    .from('charging_sessions')
    .select('id, cost, energy_kwh, charger_name, started_at')
    .eq('user_id', user.id)
    .not('cost', 'is', null)
  ...
  const transactions = (sessions || []).map(s => ({
    ...
    description: `Sesion en ${s.charger_name} - ${s.energy_kwh?.toFixed(2) || '?'} kWh`,
