...
setError('Error de red al guardar ubicación')
setSaving(false)
}
}
...
setError(null)
try {
const { data, error: sbError } = await supabase.from('chargers').select('*').order('created_at')
if (sbError) throw sbError
setChargers(data ?? [])
} catch (e: any) {
setError(e?.message || 'Error cargando cargadores')
} finally {
setLoading(false)
}
}, [])
...
setDeleting(true)
try {
const { error: sbError } = await supabase.from('chargers').delete().eq('id', id)
if (sbError) throw sbError
...
setAdding(true)
try {
const { error: sbError } = await supabase.from('chargers').insert({ id: newId.trim(), status: 'Offline' })
if (sbError) throw sbError
...
<p className="text-sm text-gray-400 mt-0.5">{loading ? '…' : `${chargers.length} cargador${chargers.length !== 1 ? 'es' : ''} registrado${chargers.length !== 1 ? 's' : ''}`}</p>
</div>
...
{/* Error banner */}
{error && (
<div className="rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-400 mb-6 flex items-center justify-between">
<span>{error}</span>
<button onClick={fetchChargers} className="ml-4 text-xs underline hover:no-underline">Reintentar</button>
</div>
)}
...
<button onClick={() => handleDelete(deleteTarget)} disabled={deleting} className="flex-1 px-4 py-2.5 bg-red-600 hover:bg-red-500 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg text-sm font-medium">
